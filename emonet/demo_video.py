...
def run_emonet(
    ...
    Returns output with added confidence and quality scores.
    """
    ...
    with torch.no_grad():
        ...
        # Add confidence and quality to output
        output["confidence"] = torch.tensor([confidence], device=device)
        output["quality"] = torch.tensor([quality], device=device)
...
def make_visualization(
    ...
    confidence = emotion_prediction.get("confidence", torch.tensor([0.0]))[0].cpu().item()
    quality = emotion_prediction.get("quality", torch.tensor([0.0]))[0].cpu().item()
    
    # Display emotion
    emotion_text = emotion_classes[predicted_emotion_class_idx]
    ...
    frame_rgb = cv2.putText(
        frame_rgb,
        confidence_text,
        ((face_bbox[0] + face_bbox[2]) // 2, face_bbox[1] + 100),
        cv2.FONT_HERSHEY_SIMPLEX,
        font_scale * 0.6,
        (0, 255, 0),
        2,
    ...
    frame_rgb = cv2.putText(
        frame_rgb,
        quality_text,
        ((face_bbox[0] + face_bbox[2]) // 2, face_bbox[1] + 150),
        cv2.FONT_HERSHEY_SIMPLEX,
        font_scale * 0.6,
        (0, 255, 255),
        2,
